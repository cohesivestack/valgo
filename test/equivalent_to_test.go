...
func TestEquivalentToValid(t *testing.T) {
	valgo.ResetMessages()
	...
	for description, values := range map[string][]interface{}{
		...
		v := valgo.Is(valueA).EquivalentTo(valueB)
		assert.True(t, v.Valid(), msg)
		assert.Empty(t, v.Errors(), msg)
	}
}

func TestEquivalentToInvalid(t *testing.T) {
	valgo.ResetMessages()
...
func TestNotEquivalentToValid(t *testing.T) {
	valgo.ResetMessages()

	v := valgo.Is(10).NotEquivalentTo(11)
	assert.True(t, v.Valid())
	assert.Empty(t, v.Errors())
}

func TestNotEquivalentToInvalid(t *testing.T) {
	valgo.ResetMessages()

	v := valgo.Is(10).NotEquivalentTo(10)
	assert.False(t, v.Valid())
	if assert.NotEmpty(t, v.Errors()) {
